...
public class MatrixDataHandler {
    ...
    public MatrixDataHandler(int h, int w, String filename) {
        initializeMatrix(h,w,filename); //metoda de initializare matrice
        writeMatrixToFile();  //metoda pentru scrierea in fisier a matricii
    ...
    public int getValueFromPosition(String filename, int posH, int posW) {
        try {
            ...
            readMatrixFromFile(); //metoda pentru citirea matricii din fisier

            if(posH < h-1 && posW < w-1)  //verificare index posH < H-1   posW < W-1
    ...
    public void modifyValueAndUpdateFile(String filename, int posH, int posW, int val) {
        try {
            ...
            initializeMatrix(h,w,filename);  //apelare metoda de reinitializare a matricii
            ...
            writeMatrixToFile();  //metoda pentru scrierea matricii in fisier
        } catch (IOException exception) {
            System.out.println(exception.getMessage());
        }
    }
